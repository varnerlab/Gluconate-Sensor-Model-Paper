...
Black circles represent experimental data with standard errors captured by error bars across at least three replicates. mRNA was measured using qPCR; Venus protein was measured on a plate reader (513 nm excitation/531 nm emission). The ParetoEnsembles.jl parameter ensemble ($N = 567$) was used to calculate the mean (black line) and 95\% confidence estimate of the model simulation (shaded region). GntR protein (panel D) was predicted by the model and not experimentally measured.}\label{fig:model_training}
\end{figure}
...
\end{center}
\caption{Predicted trajectories of consumable resource availability over the 12-hour cell-free reaction. The transcriptional resource variable $\epsilon_X$ (representing the fractional availability of nucleotides and energy cofactors for transcription) and the translational resource variable $\epsilon_L$ (representing the fractional availability of amino acids and energy cofactors for translation) were computed from the mechanistic resource depletion model (Eqns.~\ref{eq:epsX}--\ref{eq:epsL}). By 12 hours, $\epsilon_X$ declined to 0.035 (97\% depletion) while $\epsilon_L$ declined to 0.98 (2\% depletion), indicating that transcriptional resources were the primary bottleneck. Lines represent the ensemble mean; shaded regions represent the 95\% confidence interval across the parameter ensemble ($N = 567$).}\label{fig:resource_depletion}
\end{figure}
...
0 (control) & $1.54 \pm 0.08$ & 1.00$\times$ & -- & $1.54 \pm 0.08$ & 1.00$\times$ & -- \\
...
& & & & \\
cRNAP + P70 pseudo-energy & $\epsilon_{P70,cRNAP}$ & $0.070 \pm 0.16$ & $k_BT$ & -- \\
hRNAP + P70 pseudo-energy & $\epsilon_{P70,hRNAP}$ & $-0.030 \pm 0.044$ & $k_BT$ & -- \\
cRNAP + mP70 pseudo-energy & $\epsilon_{mP70,cRNAP}$ & $2.50 \pm 1.22$ & $k_BT$ & -- \\
hRNAP + mP70 pseudo-energy & $\epsilon_{mP70,hRNAP}$ & $-0.032 \pm 0.035$ & $k_BT$ & -- \\
GntR + mP70 operator & $\epsilon_{mP70,GntR}$ & $-1.34 \pm 0.53$ & $k_BT$ & -- \\
& & & & \\
Hill coeff. P70 + hRNAP & $n_{P70,hRNAP}$ & $1.22 \pm 1.00$ & -- & -- \\
Dissoc. const. P70 + hRNAP & $K_{P70,hRNAP}$ & $0.57 \pm 1.20$ & $\mu$M & -- \\
Hill coeff. mP70 + hRNAP & $n_{mP70,hRNAP}$ & $3.06 \pm 2.80$ & -- & -- \\
Dissoc. const. mP70 + hRNAP & $K_{mP70,hRNAP}$ & $1.54 \pm 3.30$ & $\mu$M & -- \\
Hill coeff. mP70 + GntR & $n_{mP70,GntR}$ & $3.87 \pm 2.49$ & -- & -- \\
Dissoc. const. mP70 + GntR & $K_{mP70,GntR}$ & $0.0042 \pm 0.0049$ & $\mu$M & -- \\
& & & & \\
TX time const. GntR & $\tau_{X,GntR}$ & $0.066 \pm 0.041$ & -- & -- \\
TX time const. Venus & $\tau_{X,Venus}$ & $0.0018 \pm 0.00088$ & -- & -- \\
TL time const. GntR & $\tau_{L,GntR}$ & $11.0 \pm 11.2$ & -- & -- \\
TL time const. Venus & $\tau_{L,Venus}$ & $0.45 \pm 0.43$ & -- & -- \\
Deg. modifier mRNA GntR & $\delta_{m,GntR}$ & $0.031 \pm 0.018$ & -- & -- \\
Deg. modifier mRNA Venus & $\delta_{m,Venus}$ & $20.3 \pm 16.5$ & -- & -- \\
Deg. modifier protein GntR & $\delta_{p,GntR}$ & $0.0030 \pm 0.0053$ & -- & -- \\
Deg. modifier protein Venus & $\delta_{p,Venus}$ & $0.0026 \pm 0.0027$ & -- & -- \\
Deg. modifier protein $\sigma_{70}$ & $\delta_{p,\sigma_{70}}$ & $0.0037 \pm 0.011$ & -- & -- \\
Translation saturation const. & $K_L$ & $456 \pm 240$ & $\mu$M & -- \\
& & & & \\
Hill coeff. gluconate-GntR & $n_{\text{gluconate}}$ & $3.19 \pm 0.41$ & -- & -- \\
Dissoc. const. gluconate-GntR & $K_{\text{gluconate}}$ & $0.14 \pm 0.02$ & mM & -- \\
& & & & \\
TX resource consumption rate & $\alpha_X$ & $(9.1 \pm 5.9) \times 10^{-5}$ & $(\mu\text{M}\cdot\text{nt}\cdot\text{hr})^{-1}$ & -- \\
TL resource consumption rate & $\alpha_L$ & $(4.5 \pm 9.2) \times 10^{-6}$ & $(\mu\text{M}\cdot\text{aa}\cdot\text{hr})^{-1}$ & -- \\
\bottomrule
\end{tabular}}
