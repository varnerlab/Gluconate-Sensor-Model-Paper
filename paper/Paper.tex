...
\doublespacing
\begin{titlepage}
{\par\centering\textbf{\Large {Resource Competition Modeling of a Cell-Free Gluconate Biosensor}}}
\vspace{0.05in}
...
\vspace{0.1in}
{\par \centering \textbf{Running Title:}~Resource Competition Model for Cell-Free Biosensors}
\vspace{0.1in}
...
\section*{Abstract}
Cell-free systems are a promising platform for prototyping biosensor circuits because they eliminate the confounding effects of cellular growth and membrane transport. However, the finite transcriptional and translational resources in these reactions impose time-dependent capacity constraints that empirical decay functions struggle to capture. We developed a resource competition model for a D-gluconate biosensor circuit in the reconstituted cell-free system PURExpress. The circuit used the transcription factor GntR to repress expression of the fluorescent reporter Venus, with D-gluconate relieving repression in a dose-dependent manner. The model comprised two layers: the first described RNA polymerase and ribosome allocation among competing genes using Michaelis-Menten-like expressions, and the second captured irreversible depletion of consumable resources (nucleotides, energy cofactors, and amino acids) through differential equations driven by cumulative transcription and translation flux. This formulation replaced the ad hoc decay functions common in cell-free models with a physically interpretable framework that simultaneously reduced the number of fitted parameters. Model parameters were estimated via multi-objective optimization in ParetoEnsembles.jl, yielding an ensemble that captured the transcription and translation kinetics across training conditions. The model identified transcriptional resources as the dominant bottleneck and, without re-estimation of parameters, accurately predicted the dose-response relationship over four orders of magnitude of D-gluconate concentration. These results demonstrate that resource competition modeling improves both the predictive capability and interpretability of cell-free biosensor models and provides quantitative insight into the allocation of transcriptional and translational capacity.
...
\end{equation}
where $R_{L,\text{free}}$ denotes the free ribosome concentration, $\dot{v}_L$ denotes the translation elongation rate (aa/s), $l_{P,j}$ denotes the length of protein $j$ in amino acids, and $K_P$ is a polysome amplification constant (set to 1 in this study).
...
\subsubsection*{Resource competition model.}
A key contribution of this study was the replacement of ad hoc resource depletion functions (e.g., logistic or exponential decay in time) with a two-layer resource competition framework. The first layer described the allocation of transcriptional and translational machinery between competing genes. The free RNA polymerase and ribosome concentrations were computed algebraically from the total concentrations $R_{X,T}$ and $R_{L,T}$ by accounting for the fraction of each machinery sequestered by active genes:
\begin{eqnarray}
\label{eq:RNAP_free}
...
\section*{Discussion}
This study demonstrated that a resource competition model provided an improved and physically interpretable framework for describing the dynamics of a cell-free biosensor circuit. The two-layer formulation, comprising machinery allocation and consumable resource depletion, replaced the ad hoc logistic and exponential decay functions commonly used in cell-free models with a load-dependent framework that required fewer estimated parameters. Importantly, the two-layer formulation linked resource exhaustion to the actual biosynthetic load placed on the system, rather than to a predetermined time course, enabling the model to generalize across conditions with different gene dosages or effector concentrations.
...
Analysis of parameter correlations across the ensemble revealed several structurally identifiable clusters that reflected structural coupling within the model (Fig.~\ref{fig:correlation}). The translation time constants $\tau_{\text{protein,GntR}}$ and $\tau_{\text{protein,Venus}}$ were strongly positively correlated with each other ($\rho = 0.82$) and negatively correlated with the translation saturation constant $K_L$ ($\rho = -0.85$ and $-0.84$, respectively), reflecting the well-known trade-off between Michaelis constant and time constant in enzyme kinetic expressions. The pseudo-energy for Venus transcription ($\Delta G_{\text{Venus,RNAP}}$) was strongly anti-correlated with the Venus mRNA time constant ($\rho = -0.74$), indicating a compensatory relationship between promoter strength and initiation kinetics. The GntR mRNA degradation modifier was anti-correlated with the transcriptional resource consumption rate $\alpha_X$ ($\rho = -0.72$), indicating that faster mRNA degradation could be compensated by slower resource depletion to maintain the same net mRNA output. These correlations provided useful information about the structural identifiability of the model: while individual parameter values within a correlated cluster could not be precisely determined, the constrained combinations of parameters that the data supported were biophysically meaningful.

Examination of the marginal parameter distributions across the ensemble provided insight into the robustness of the key findings (Fig.~\ref{fig:param_dist}). The resource consumption rates ($\alpha_X$ and $\alpha_L$) were tightly constrained with narrow, unimodal distributions, indicating that the transcriptional bottleneck finding --- 76\% of transcriptional resources consumed versus 2\% of translational resources --- was a robust consequence of the data rather than an artifact of parameter uncertainty. Similarly, the gluconate binding parameters ($K_{\text{gluconate}}$ and $n_{\text{gluconate}}$) were among the most tightly identified parameters in the model, which explained the high accuracy of the dose-response predictions. In contrast, the $\sigma_{70}$ binding parameters ($K_{\text{Venus},\sigma_{70}}$, $K_{\text{GntR},\sigma_{70}}$) exhibited broad distributions spanning an order of magnitude, reflecting the limited sensitivity of the model outputs to the exact value of $\sigma_{70}$ affinity when $\sigma_{70}$ was supplied in excess as a pre-formed holoenzyme. Several parameters accumulated near their bounds: the Venus mRNA degradation modifier ($\delta_{m,\text{Venus}}$) spread toward its upper bound, consistent with the rapid mRNA equilibration that produced the early Venus mRNA peak discussed above, and the GntR repression Hill coefficient ($n_{\text{Venus,GntR}}$) piled against its upper bound of 10, suggesting that the data supported strong cooperative repression beyond what the current parameterization could capture. These bound-hitting behaviors provide guidance for future model refinement and indicate structural limitations that could be resolved by modifying the functional form of the corresponding model terms.
...
Several limitations of the current study should be noted. First, the model underestimated the early rate of Venus protein accumulation (Fig.~\ref{fig:model_training}B), producing a more gradual rise than the steep initial increase observed experimentally. This discrepancy likely arose because $\sigma_{70}$ was supplied exogenously and was not produced by the circuit, so the transcriptional drive was approximately constant from $t = 0$ rather than building up over time as it would in an extract-based system with endogenous $\sigma_{70}$ production. Incorporating $\sigma_{70}$ as a dynamically produced species, rather than a fixed exogenous input, could improve the early-time protein kinetics in future model iterations. Second, GntR protein was not directly measured, so the predicted GntR protein trajectory could not be experimentally validated. While the resource competition model provided a natural bound on GntR accumulation, direct measurement of GntR protein (e.g., via Western blot or mass spectrometry) would strengthen confidence in the model predictions. Third, the model did not capture the inhibitory effect of high D-gluconate concentrations (above 20 mM) on the PURExpress system, which was observed experimentally but not included in the model formulation. Incorporating an inhibition term for high effector concentrations would extend the predictive range of the model but was beyond the scope of this study. Fourth, the $\alpha_X$ and $\alpha_L$ parameters, while physically motivated, were empirical consumption rate constants that aggregated many underlying biochemical processes (nucleotide depletion, energy cofactor exhaustion, byproduct accumulation) into single parameters. A more detailed model of the metabolic network underlying transcription and translation in PURExpress could replace these lumped parameters with explicit metabolite balances, as has been done for extract-based systems using metabolomic measurements \cite{Vilkhovoy:2019aa,Horvath2020}, but would require additional experimental measurements to constrain the expanded parameter space. Recent advances in LC/MS-based metabolite quantification in cell-free reactions \cite{Vilkhovoy:2019aa} could provide the data needed to decompose the lumped $\alpha$ parameters into individual metabolite consumption terms in future work.

\section*{Conclusions}
In this study, we developed and validated a resource competition model for a D-gluconate responsive biosensor circuit operating in the reconstituted cell-free system PURExpress. The model incorporated a two-layer resource framework: machinery allocation described the distribution of RNA polymerase and ribosome between competing genes, while consumable resource depletion captured the irreversible exhaustion of nucleotides, energy cofactors, and amino acids through differential equations driven by cumulative biosynthetic flux. This formulation replaced ad hoc empirical decay functions with a load-dependent framework, and provided a physical interpretation of the resource limitations observed in cell-free reactions. The model accurately captured the transcription and translation kinetics under training conditions and, without re-estimation of parameters, predicted the dose-response relationship of Venus protein as a function of D-gluconate concentration over four orders of magnitude. The resource depletion analysis revealed that transcriptional resources were the primary bottleneck in the PURExpress system, with 76\% consumed by 12 hours compared to 2\% for translational resources. These findings provide quantitative design rules for allocating transcriptional and translational capacity in synthetic gene circuits and demonstrate the value of resource competition models for improving the predictive capability of cell-free biosensor frameworks.
...
\end{center}
\caption{Resource competition model simulations versus experimental measurements for the de-repressed training condition. Linear DNA used were: mP70-Venus (7 nM), P70-GntR (10 nM). D-gluconate (10 mM) was added as the effector at the start of reaction.
\textbf{A}: Venus mRNA.
...
\begin{figure}[h!]
\begin{center}
\includegraphics[width=1\textwidth]{./figs/fig_pareto_fronts.pdf}
\end{center}
\caption{Pairwise projections of the six-dimensional Pareto front. Grey points represent the 41,280 archive solutions with total error $< 100$ (out of 73,747 total); blue points represent the $N = 78$ filtered ensemble members. \textbf{A}: Venus mRNA SSE versus Venus protein SSE. \textbf{B}: Venus protein SSE at 10 mM versus no-GntR control. \textbf{C}: Venus protein SSE at 10 mM versus 0 mM (repressed). \textbf{D}: Venus mRNA SSE versus GntR mRNA SSE. The filtered ensemble occupies the corner of the Pareto front where all objectives are simultaneously minimized.}\label{fig:pareto_fronts}
\end{figure}
...
\begin{figure}[h!]
\begin{center}
\includegraphics[width=1\textwidth]{./figs/fig_parameter_distributions.pdf}
\end{center}
\caption{Marginal parameter distributions across the filtered ensemble ($N = 78$). Each panel shows the histogram for one of the 25 estimated parameters. Black vertical lines indicate the ensemble mean; red dashed lines indicate the parameter bounds (where visible within the axis range). Parameters with narrow, unimodal distributions (e.g., $K_{\text{gluconate}}$, $n_{\text{gluconate}}$, $\alpha_X$) are well-identified by the data, while parameters with broad distributions (e.g., $K_{\text{Venus},\sigma_{70}}$, $\delta_{p,\text{Venus}}$) or distributions piling against bounds (e.g., $\delta_{m,\text{Venus}}$, $n_{\text{Venus,GntR}}$) indicate structural identifiability limitations.}\label{fig:param_dist}
\end{figure}
