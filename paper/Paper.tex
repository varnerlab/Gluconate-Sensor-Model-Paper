...
Examination of the marginal parameter distributions across the ensemble revealed a spectrum of identifiability (Fig.~\ref{fig:param_dist}). The gluconate binding parameters ($K_{\text{gluconate}}$ and $n_{\text{gluconate}}$), the resource consumption rates ($\alpha_X$ and $\alpha_L$), and the GntR mRNA degradation modifier ($\delta_{m,\text{GntR}}$) were tightly constrained with narrow, unimodal distributions, indicating that these parameters were well-identified by the data. In contrast, the $\sigma_{70}$ binding parameters ($K_{\text{Venus},\sigma_{70}}$, $K_{\text{GntR},\sigma_{70}}$) exhibited broad distributions spanning an order of magnitude, reflecting the limited sensitivity of the model outputs to the exact value of $\sigma_{70}$ affinity when $\sigma_{70}$ was supplied in excess. Several parameters accumulated near their bounds: the Venus mRNA degradation modifier ($\delta_{m,\text{Venus}}$) spread toward its upper bound, consistent with the fast mRNA equilibration discussed above, and the GntR repression Hill coefficient ($n_{\text{Venus,GntR}}$) piled against its upper bound of 10, suggesting that the data supported strong cooperative repression. These bound-hitting behaviors provide guidance for future model refinement: parameters that accumulate at bounds may indicate structural limitations that could be resolved by modifying the functional form of the corresponding model term.
